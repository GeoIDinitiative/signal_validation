...
Figures (written next to this script):
  01_records_overview.png       raw tilt-x over each full record (winter vs summer)
...
WINTER = ("EEC1 (winter / INGV)", "/home/owen/Signals/experiment/INGV/EEC1.feather", "f", "#1d4ed8")
SUMMER = ("EC1 (summer / experiment)", "/home/owen/Signals/experiment/EC1.csv", "c", "#dc2626")
...
def main():
    ...
    ax.set_title("In-band tilt background per period (winter/summer ratio annotated)", fontweight="700", fontsize=13)
    ...
    ax.set_title("Broadband PSD — winter vs summer (tide = internal calibrator; bands diverge in the search band)",
    ...
    ax.set_title("In-band tilt-x amplitude distribution (winter is broader → larger excursions)",
    ...
    ax[0].set_xticks([0, 1]); ax[0].set_xticklabels(["winter", "summer"])
    ax[0].set_ylabel("in-band variance explained by na (%)", fontweight="700")
    ax[0].set_title(f"Thermal coupling: r(tilt,na) winter={wr:+.2f} summer={sr:+.2f}", fontweight="700", fontsize=12)
    ax[0].grid(axis="y", alpha=0.3)
    ax[1].bar([0, 1], [wlsb, slsb], color=[WINTER[3], SUMMER[3]])
    ax[1].set_xticks([0, 1]); ax[1].set_xticklabels(["winter", "summer"])
    ...
    fig.suptitle("Why the winter background is real, not instrumental", fontweight="700", fontsize=14)
    fig.tight_layout(); fig.savefig(HERE/"06_thermal_and_units.png", dpi=300); plt.close(fig)

    # 07 — detection / template performance / upper limit (EEC1 vs EC1)
    fig, ax = plt.subplots(1, 3, figsize=(15, 5))
    try:
        sst = pd.read_csv(TV/"SWCC_comprehensive/top_templates/sst_peak_counts.csv")
        for c, st, col in [(0, "EEC1", WINTER[3]), (1, "EC1", SUMMER[3])]:
            pass
        for stn, col, off in [("EEC1", WINTER[3], -0.2), ("EC1", SUMMER[3], 0.2)]:
            g = sst[sst.station == stn].groupby("template").agg(n=("n_all", "sum")).reindex(
        ...
        ax[0].set_ylabel("matched peaks (|r|≥0.2)", fontweight="700"); ax[0].set_title("Template peak yield", fontweight="700")
    ...
    try:
        ...
        ax[2].set_xticks([0, 1]); ax[2].set_xticklabels(["EEC1\n(winter)", "EC1\n(summer)"])
        ax[2].set_ylabel("A_min peak tilt (90% conf.)", fontweight="700")
        ax[2].set_title("Detection upper limit\n(noisier winter → weaker bound)", fontweight="700")
        ax[2].grid(axis="y", alpha=0.3)
    except Exception as e:
        ax[2].text(0.5, 0.5, f"upper-limit data n/a\n{e}", ha="center")
    fig.suptitle("EEC1 vs EC1 — template performance and detection sensitivity", fontweight="700", fontsize=14)
    fig.tight_layout(); fig.savefig(HERE/"07_detection_performance.png", dpi=300); plt.close(fig)

    L = ["EEC1 (winter) vs EC1 (summer) — SAME borehole instrument, two campaigns", "=" * 66,
